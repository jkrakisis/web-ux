...
Web UX Archive는 GDWEB의 WEB 부문 신규 선정작을 평일마다 확인하고, 공개된 근거를 바탕으로 UX 패턴을 분석한 뒤 Notion과 웹 대시보드에 축적하는 개인 UX 리서치 파이프라인이다.

초기의 흐름은 `GPT 예약 실행 → Notion 저장`이었다. 현재는 `GitHub Actions 자동 수집 → 근거 기반 분석 → Notion 등록 및 전체 이력 동기화 → GitHub Pages 탐색` 구조로 발전했다.
...
flowchart LR
    ...
    Live --> Evidence
    Evidence --> Analysis["OpenAI 분석 또는 폴백 분석"]
...
| `src/gdweb_daily/analysis.py` | OpenAI 분석, 폴백 분석, 고정 6줄 포맷 생성 |
...
`docs/data/latest.json`의 핵심 구조는 다음과 같다.
...
`new_count`는 이번 실행 결과이고 `total_count`는 누적 이력이다. 이 둘을 분리함으로써 `신규 없음`과 `아카이브가 비어 있음`을 다른 상태로 표현한다.

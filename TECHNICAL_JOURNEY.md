...
| 자동 실행 | 한국시간 기준 평일 18:30 예약 + 수동 실행 |
| 최신 검증 결과 | 누적 63건, 등록일 28일, 드라이런 신규 2건, 실패 0건 |
...
### 4.8 실행 시간과 분석 정보 구조 개선

하루 1회 실행을 유지하면서 당일 신규 항목 포착률을 높이기 위해 예약 시간을 08:37에서 18:30으로 변경했다. 2026-07-29 오전 실행은 10:01에 시작되어 당시 확인된 항목을 처리했지만, 이후 같은 등록일의 사이트 2건이 추가됐다. GDWEB 안내 운영시간 10:00–18:00 이후에 실행하면 이런 늦은 등록을 같은 날 수집할 가능성이 높아진다.

대시보드의 6번 분석은 기술 키워드와 A–D 제안이 한 문장으로 이어져 읽기 어려웠다. 이를 기술·플러그인, IA 퀵액션, KPI, 공공기관 Do/Don’t, 오늘의 한 줄의 다섯 영역으로 분리했다. 기술 키워드는 칩으로 표시하고, 데스크톱에서 A·B는 2열, C·D는 각각 전체 너비, 모바일은 전체 1열로 구성했다. 이전 축약형 데이터도 같은 구조로 보이도록 호환 파서를 추가했다.

### 4.9 최근 10건 썸네일 시험

2026-08-09 기준 최신 사이트 10건에 실사이트 첫 화면 캡처를 시험 적용했다. 전체 페이지가 아니라 1440×900 첫 화면만 저장하고, `networkidle` 대신 `domcontentloaded` 이후 짧은 안정화 대기를 사용했다. 영상과 주요 분석 요청을 차단하고 사이트별 실패를 격리해 캡처 오류가 수집·Notion 등록·Pages 배포를 막지 않도록 했다.

시험 결과는 8건 성공, 시각적 빈 화면 1건, HTTP 403 차단 1건이었다. 성공한 항목에만 `thumbnail_url`을 기록한다. 실패 항목은 제목만 표시하며 이미지 공간을 만들지 않고, 이후 이미지 파일이 깨지는 경우에도 프런트엔드가 썸네일 래퍼를 제거한다.
...
`docs/data/latest.json`의 핵심 구조는 다음과 같다. 아래 코드는 현재 `items` 누적 데이터 배열 63건 중 1건만 구조 설명을 위해 발췌한 예시다.
...
- 실행 시각: 한국시간 평일 18:30
- 선정 이유: GDWEB 안내 운영시간 종료 후 당일 등록분을 최대한 포함
- 지연 대응: GitHub Actions 시작 지연을 허용하고 최근 7일을 중첩 조회
- 중복 대응: `str_no`, 또는 `도메인 + 등록일` 조합으로 재처리 방지
- 보충 실행: 긴급 확인이 필요하면 `workflow_dispatch`에서 `dry_run` 또는 실등록 모드로 수동 실행
...
### 13.5 오전 1회 실행 이후 당일 추가 등록 누락

2026-07-29 예약 실행은 10:01에 시작해 성공했지만, 실행 이후 GDWEB WEB 목록에 같은 날짜의 `센트레빌 아스테이룸 거제`와 `이스트소프트`가 추가됐다. 실행 자체는 성공했어도 하루 중 등록 시점보다 이르면 당일 항목을 놓칠 수 있다는 운영상 공백이었다.

해결:

- 하루 1회 예약을 평일 18:30으로 이동
- 최근 7일 중첩 조회를 유지해 다음 실행에서도 누락분 복구
- 체크포인트는 수집과 실등록이 모두 성공한 경우에만 갱신
- 필요 시 수동 실행으로 즉시 보충하되 기존 중복 검사 재사용

### 13.6 기술/플러그인과 A–D 제안의 낮은 가독성

6번 항목이 하나의 긴 문장으로 표시되어 기술 근거와 실행 제안을 구분하기 어려웠다.

해결:

- 기술 키워드를 개별 칩으로 분리
- A · IA 퀵액션과 B · KPI를 2열 카드로 구성
- C · 공공기관 Do/Don’t와 D · 오늘의 한 줄을 전체 너비 카드로 구성
- 780px 이하에서는 모든 카드를 1열로 전환
- 최신 형식 60건과 과거 축약형 형식을 모두 처리하는 호환 파서 적용
...
### 2026-07-29 운영 및 UI 개선 검증

| 검증 항목 | 결과 |
| --- | --- |
| 오전 예약 실행 | 성공, 10:01 시작 |
| 현재 GDWEB 목록 대조 | 같은 날짜 후속 등록 2건 확인 |
| 예약 시간 변경 | 평일 18:30 KST |
| 누적 공개 데이터 | 63건, 등록일 28일 |
| 최신 드라이런 | 신규 2건, 실패 0건 |
| 6번 형식 파싱 | 데이터가 있는 60건 전부 성공 |
| Python 테스트 | 10 passed |
| JavaScript 문법 검사 | 통과 |
| 데스크톱 UI | 기술 칩, A·B 2열, C·D 전체 너비 |
| 모바일 UI | 1열 전환, 가로 넘침 없음 |
| GitHub Pages 배포 | 성공 |

관련 실행:

- [오전 예약 실행 30412857531](https://github.com/jkrakisis/web-ux/actions/runs/30412857531)
- [UI 최종 배포 30456313982](https://github.com/jkrakisis/web-ux/actions/runs/30456313982)
...
- Notion 이력과 신규 수집 결과를 병합한 웹 아카이브 63건을 등록일별로 빠르게 탐색
